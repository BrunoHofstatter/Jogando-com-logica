...
import {
  ...
  getRooms,
  setRoom,
} from "../rooms/roomStore.ts";

type CrownChaseServer = Server<
  CrownChaseClientToServerEvents,
  CrownChaseServerToClientEvents
>;
...
export function registerRoomHandlers(io: CrownChaseServer): void {
  io.on("connection", (socket) => {
    socket.on("create_room", ({ playerName }) => {
      ...
      const roomCode = generateRoomCode(new Set(getRooms().keys()));
...
function leaveAnyExistingRoom(
  ...
  const currentRoom = findRoomBySocketId(socket.id);
  if (!currentRoom) {
    return;
  }
