...
const SESSION_STORAGE_KEY = "mathwar_multiplayer_session_v1";
...
function getNamespaceUrl(serverUrl: string): string {
  return `${serverUrl.replace(/\/$/, "")}/math-war`;
}
...
export function useMathWarMultiplayer() {
  ...
  return {
    ...
    submitMove,
    requestRematch,
    leaveRoom: leaveMathWarMultiplayerRoom,
  };
}
