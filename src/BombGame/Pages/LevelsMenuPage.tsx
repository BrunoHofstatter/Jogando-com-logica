...
function LevelsMenuPage() {
  ...
  return <div className={styles.gamePageContainer}>
    ...
    <div className={styles.levelsScrollWrapper}><div className={styles.levelsGrid}>
      <button className={`${styles.levelButton} ${styles.unlocked}`} onClick={() => navigate(ROUTES.BOMB_GAME_MP_LOBBY)}><span className={styles.levelNumber}>1</span><span className={styles.bombgameDescription}>Números</span></button>
      {[2, 3, 4, 5].map((level) => <button key={level} disabled className={`${styles.levelButton} ${styles.locked}`}><span className={styles.levelNumber}>{level}</span><LockKeyhole className={styles.lockIcon} /></button>)}
    </div></div>
